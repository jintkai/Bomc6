...
public class KbpFrameTest extends TestCase {
    ...
    public void searchKbpByTreeCaption(String[] obj)
    {
        ...
        Assert.assertEquals(gridTable.getRowNum(), Integer.parseInt(obj[3]), "KBPTree查询条数错误");
        Assert.assertTrue(gridTable.equalsSearch("./td[3]", gridTable.getRowNum(), obj[5]), "KBPTree查询结果错误");
    }
    ...
    public void searchKbpByTreeClass(String[] obj)
    {
        GridTablePage gridTable=kbpFrame.searchKbpByTree(obj[5]);
        Assert.assertEquals(gridTable.getRowNum(), Integer.parseInt(obj[3]), "KBPTree查询条数错误");
        Assert.assertTrue(gridTable.equalsSearch("./td[2]", gridTable.getRowNum(), obj[5]), "KBPTree查询结果错误");
    }
    ...
    public void searchByClass(String[] obj)
    {
        ...
        Assert.assertEquals(gridTable.getRowNum(), Integer.parseInt(obj[3]), "KbpClass查询");
        Assert.assertTrue(gridTable.equalsSearch("./td[2]", gridTable.getRowNum(), obj[5]), "KbpClass查询");
    }
    ...
    public void searchByMulti(String[] obj)
    {
        ...
        Assert.assertTrue(gridTable.equalsSearch("./td[2]",Integer.parseInt(obj[3]),obj[5]),"searchMulit,用例失败，Case ID:"+obj[1]);
        Assert.assertTrue(gridTable.equalsSearch("./td[3]",Integer.parseInt(obj[3]),obj[6]),"searchMulit,用例失败，Case ID:"+obj[1]);
    }
